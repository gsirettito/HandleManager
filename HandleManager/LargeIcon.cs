...
public static class LargeIcon {
    ...
    //Any size
    private static IntPtr AnySize(int iImage, string ID_IImageList, 
        int imageSize) {
        IImageList spiml = null;
        Guid guil = new Guid(ID_IImageList);//or IID_IImageList

        Shell32.SHGetImageList(imageSize, ref guil, ref spiml);
        IntPtr hIcon = IntPtr.Zero;
        spiml.GetIcon(iImage, Shell32.ILD_TRANSPARENT | Shell32.ILD_IMAGE, ref hIcon); //
    ...
    public static Icon ExtractAssociatedIcon(string fileName, IconSize iconSize) {
        IntPtr hIcon = IntPtr.Zero;
        if (iconSize == IconSize.Size32x32)
            hIcon = AnySize(GetIconIndex(fileName), IID_IImageList, Shell32.SHIL_SMALL);
        else if (iconSize == IconSize.Size48x48)
            hIcon = AnySize(GetIconIndex(fileName), IID_IImageList, Shell32.SHIL_EXTRALARGE);
        else if (iconSize == IconSize.Size128x128)
            hIcon = AnySize(GetIconIndex(fileName), IID_IImageList, Shell32.SHIL_LARGE);
        Icon ico = (Icon)System.Drawing.Icon.FromHandle(hIcon).Clone();
        return ico;
    }
